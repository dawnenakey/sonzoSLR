...
def handle_get_videos(cors_headers):
    ...
    try:
        ...
        table = dynamodb.Table(os.environ.get('DYNAMODB_TABLE', 'spokhand-sessions'))
        ...
        for item in response.get('Items', []):
            ...
            if 'video_id' in item and 'filename' in item:
                video = {
                    ...
                    'url': f"https://{os.environ.get('S3_BUCKET_NAME', 'spokhand-videos')}.s3.amazonaws.com/{item.get('video_id', '')}"
                }
                videos.append(video)
