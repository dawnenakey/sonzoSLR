...
def lambda_handler(event, context):
    ...
    try:
        ...
        # For proxy resources, we need to construct the full path
        resource_path = event.get('requestContext', {}).get('resourcePath', '')
        path_parameters = event.get('pathParameters', {})
        
        # If it's a proxy resource, construct the actual path
        if '{proxy+}' in resource_path:
            proxy_value = path_parameters.get('proxy', '')
            # Remove the {proxy+} part and replace with actual proxy value
            actual_path = resource_path.replace('{proxy+}', proxy_value)
        else:
            actual_path = resource_path

        logger.info(f"HTTP Method: {http_method}, Resource Path: {resource_path}, Actual Path: {actual_path}")

        if http_method == 'OPTIONS':
            return create_response(200, 'CORS preflight OK', cors_headers)
        
        elif http_method == 'POST' and actual_path == '/sessions':
            return handle_create_session(event, cors_headers)

        elif http_method == 'POST' and actual_path.startswith('/sessions/') and actual_path.endswith('/upload-video'):
            return handle_generate_upload_url(event, cors_headers)

        elif http_method == 'GET' and actual_path == '/sessions':
            return handle_get_sessions(event, cors_headers)

        elif http_method == 'GET' and actual_path.endswith('/annotations'):
            return handle_get_annotations(event, cors_headers)

        elif http_method == 'POST' and actual_path.endswith('/annotations'):
            return handle_create_annotation(event, cors_headers)
            
        else:
            logger.warning(f"No route matched for method '{http_method}' and path '{actual_path}'")
...
def handle_get_annotations(event, headers):
    ...
    try:
        # For proxy resources, extract video_id from the path
        path_parameters = event.get('pathParameters', {})
        proxy_value = path_parameters.get('proxy', '')
        
        # Parse the proxy path to extract video_id
        # Expected format: uploads/{session_id}/{filename}/annotations
        path_parts = proxy_value.split('/')
        if len(path_parts) >= 3 and path_parts[-1] == 'annotations':
            # Construct video_id from the path parts
            video_id = '/'.join(path_parts[:-1])  # Everything except 'annotations'
        else:
            return create_response(400, {'success': False, 'error': 'Invalid path format for annotations.'}, headers)
